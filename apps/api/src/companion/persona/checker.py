...
class PersonaConsistencyChecker:
    ...
    async def guard(self, *, session_id: UUID, draft: str) -> PersonaGuardResult:
        try:
            claims = await self._extract(draft)
        ...
        try:
            repaired_claims = await self._extract(repaired_draft)
    ...
    async def _extract(self, draft: str) -> list[CompanionClaim]:
    ...
    def _conflicts(
        ...
        for claim in claims:
            ...
            if self._normalize(expected) != self._normalize(claim.value):
                conflicts.append(
                    PersonaConflict(
                        predicate=claim.predicate,
                        expected=expected,
